...
let wasMoving = false; // NEW: A flag to detect when the player stops moving
...
const spawnPoints = [
  { x: 400, y: 300 }, { x: 150, y: 100 }, { x: 150, y: 450 }, { x: 650, y: 100 }, { x: 650, y: 450 }
];
...
function checkCollision(rect1, rect2) { /* ... unchanged ... */ }

// REWRITTEN: A cleaner, more robust handleMovement function
function handleMovement() {
    ...
    // 1. Get Input (The duplicated block is now removed)
    ...
    // 2. Update Position and Direction if moving
    if (isMoving) {
        // --- Collision Detection ---
    ...
    // 3. Constrain to canvas edges
    player.x = constrain(player.x, 0, width - SPRITE_W);
    player.y = constrain(player.y, 0, height - SPRITE_H);

    // 4. Send Update to Supabase if moving OR if we just stopped moving
    ...
    // 5. Update the wasMoving flag for the next frame
    wasMoving = isMoving;
}

// --- (Paste your full, unchanged functions from here down) ---
function preload() {
